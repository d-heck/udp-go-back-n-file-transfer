...
// BONUS Client Side Implementation Of UPD File Transfer with Go-Back-N pipeline
		...
		if ((strncmp(messageBuffer, "OK", 2)) == 0) {
			...
			if(clientFile == NULL){
				...
				while(transferFlag == 1){
					...
					// Determine based on probability q if client will drop the packet
