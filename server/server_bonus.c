...
// BONUS Server Side Implementation Of UPD File Transfer with Go-Back-N pipeline
	...
	while(1) { 
		...
		if(serverFile != NULL){
			...
			if ((strncmp(messageBuffer, "OK", 2)) == 0) {
				...
				clock_t packetTimerStart, packetTimerEnd; // Used for timeout timing (Go-Back-N)
				double packetTime; // Used to check if a timeout has occured
				...
				while(transferFlag == 1){
					...
					for(int i = 0; i < windowSize; i++){
						...
						// If last received ack is not for the last in flight segment, check if a timeout occurred
						if (lastAck != lastSegment){
							packetTimerEnd = clock();
							packetTime = (double)(end - begin) / CLOCKS_PER_SEC;

							// Check if packetTime exeeded timeout interval. If so, detect packet drop.
							if (packetTime > timeOutInterval){
								printf("SERVER: TIME-OUT. Packet drop detected! Resending segment %d \n", lastSegment);
								break; // Slide window to last ack-ed packet, resend necessary packet.
							}
						}
...
int main(int argc, char* argv[]){
	...
	float timeOutInterval; // Used to determine time out period for dropped packets.
	...
	if (argv[1] == NULL){
		...
		timeOutInterval = .5; // Arbitrary half second timeout interval
 	}
	else{
		port = argv[1];
		windowSize = atoi(argv[2]);
		errorProbability = atof(argv[3]);	
		timeOutInterval = fabs(atof(argv[3]));		
	}
